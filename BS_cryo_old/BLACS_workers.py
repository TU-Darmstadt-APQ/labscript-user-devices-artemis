...
import threading
from .utils import _get_channel_num


class BS_cryoWorker(Worker):
    def init(self):
        ...
        try:
            ...
            self.bias_supply = BiasSupply(self.port, self.baud_rate, self.supports_custom_voltages_per_channel,
                                                self.default_voltage_range, self.AO_ranges)

        except LabscriptError as e:
            raise RuntimeError(f"BS-cryo bias supply identification failed: {e}")
        except Exception as e:
            raise RuntimeError(f"An error occurred during BS_cryoWorker initialization: {e}")
        ...
        if not getattr(self, 'restored_from_final_values', False):
            if self.verbose is True:
                ...
                for ch_name, voltage in front_panel_values.items():
                    print(f"  {ch_name}: {voltage:.2f} V") # CH 10 0.200
