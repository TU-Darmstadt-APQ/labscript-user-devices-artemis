...
class Caen:
        ...
        try:
            ...
            for p in ports:
                if self.pid.upper() in p.hwid and self.vid.upper() in p.hwid:
                    ...
                    cmd_ser = "$CMD:MON,PAR:BDSNUM"
                    data = (cmd_ser + '\r\n').encode('utf-8')
                    tmp_serial.write(data)
                    response = tmp_serial.readline()
                    serial_no = self._parse_response(response, expect_val=True, str_exp=True)

                    # response = self.query_serial(tmp_serial)
                    print(f"[DEBUG] Serial numbers: (queried, given) {repr(serial_no)}, {repr(self.serial_number)}")
    ...
    def receive_from_CAEN(self) -> bytes|None:
        try:
            if self.using_serial:
    ...
    def query(self, cmd: str, expect_val:bool=False, str_exp=False) -> str | float:
        self.send_to_CAEN(cmd)
        response = self.receive_from_CAEN()
        resp = self._parse_response(response, expect_val, str_exp)
    ...
    def _parse_response(self, response: bytes, expect_val: bool = False, str_exp=False) -> str | float | None:
        try:
            response: str = response.decode('utf-8').strip()
        except UnicodeDecodeError:
            return f'Error decoding response.'
