...
class AnalogOutStahl(AnalogOut):
    @set_passed_properties(
        property_names={"connection_table_properties": ["ao_range"]}
    )
    def __init__(self, name, parent_device, connection, ao_range=None, **kwargs):
        super().__init__(name, parent_device, connection, **kwargs)
        self.ao_range = ao_range

class Stahl_HV(IntermediateDevice):
    description = 'Stahl_HV_Series'
    ...
    def __init__(self, name, port='', baud_rate=9600, vid=None, pid=None, parent_device=None, ao_range=None, num_ao=None, serial_number=None, **kwargs):
        super().__init__(name, parent_device, **kwargs)

        if port:
            self.BLACS_connection = '%s,%s' % (port, str(baud_rate))
        elif not port and (vid and pid):
            self.BLACS_connection = '%s,%s' % (vid, pid)
        elif serial_number:
            self.BLACS_connection = '%s' % serial_number
    ...
    def generate_code(self, hdf5_file):
        ...
        for child_device in self.child_devices:
            if isinstance(child_device, (AnalogOut, AnalogOutStahl)):
    ...
    def _make_analog_out_table(self, analogs, times):
        ...
        dtypes = [('time', np.float64)] + [(c, np.float32) for c in connections]
