...
class PicoAnalogIn(AnalogIn):
    @set_passed_properties({"connection_table_properties": ["channel_config"]})  # use in BLACS_tab
    def __init__(self, name, parent_device, connection, enabled:int, coupling:str, range:float, analog_offset:float, **kwargs):
        ...
        if 0.1 > range or range > 200:
            raise ValueError(f"Invalid 'range' value: {range}. Expected between 0 and 200.")

        self.channel_config = dict(channel=connection, name=name, enabled=enabled, coupling=coupling, range=range, analog_offset=analog_offset)
...
class PicoScope4000A(Device):
    ...
    def set_simple_trigger(self, enabled:int, source:str, threshold:int, direction="rising", delay:int=0, autoTrigger_ms:int=0):
        """
        Simple edge trigger.
        ...
        :param delay: the time, in sample periods, between the trigger occurring and the first sample being taken.
        :param autoTrigger_ms: the number of milliseconds the device will wait if no trigger occurs. If 0, wait infinitely
        """

        self.simple_trigger_config.update(dict(enabled=enabled, source=source, threshold=threshold, direction=direction, delay=delay, autoTrigger_ms=autoTrigger_ms))
    ...
    def set_trigger_delay(self, delay:int):
        ...
        self.trigger_delay_config.update(dict(delay=delay))


    def set_trigger_properties(self, source:str, thresholdMode:str, thresholdUpper:float=None, thresholdLower:float=None,
                               thresholdUpperHysteresis:float=None, thresholdLowerHysteresis:float=None,
                                ):
        """
        ...
        """
        self.trigger_properties_config.append(dict(threshold_upper=thresholdUpper, threshold_lower=thresholdLower,
                                                   upper_hysteresis=thresholdUpperHysteresis,
                                                   lower_hysteresis=thresholdLowerHysteresis,
    ...
    def set_block_sampling(self, noPreTriggerSamples:int, noPostTriggerSamples:int, sampleInterval_ns:float):
        ...
        self.block_config.update(dict(preTriggerSamples=noPreTriggerSamples,
                                 ...
                                 sampling_interval=sampleInterval_ns))
    ...
    def set_rapid_block_sampling(self, noPreTriggerSamples:int, noPostTriggerSamples:int, sampleInterval_ns:int, noSegments:int):
        ...
        self.rapid_block_config.update(dict(preTriggerSamples=noPreTriggerSamples,
                                       postTriggerSamples=noPostTriggerSamples,
                                       sampleInterval_ns=sampleInterval_ns,
    ...
    def signal_generator_config(self,
        ...
        :param offsetVoltage: Voltage offset in microvolts to apply to the waveform.
        :param pkToPk:  Peak-to-peak voltage in microvolts.
        ...
        self.siggen_config.update(dict(offsetVoltage=offsetVoltage, pkToPk=pkToPk, wavetype=wavetype,
    ...
    def generate_code(self, hdf5_file):
        ...
        group.create_dataset("channel_configs", data=input_configs_table)

        # ------------------------------------------- Save triggers -------------------------------------------
        trig_group = group.create_group('triggers')

        # Simple trigger
        if self.simple_trigger_config is not None:
